#!/usr/bin/env python3
"""Compile and run each ```java snippet in bindings/java/docs/, inject ```output blocks."""
...
from runners.java import run_java

JAVA_DIR = (
    Path(sys.argv[1]).resolve()
    if len(sys.argv) > 1
    else REPO_ROOT / "bindings" / "java"
)
DOCS_DIR = JAVA_DIR / "docs"
TIMEOUT = 30
...
def process_file(md_path: Path, failures: list[str]) -> bool:
    ...
    for m in FENCE_RE.finditer(original):
        ...
        snippet = Snippet(file=md_path, line=0, lang_tag="java", tab=None, code=code)
        run_result = run_java(snippet, timeout=TIMEOUT)
