#!/usr/bin/env python3
"""Run each ```go snippet in bindings/go/docs/, inject ```output blocks."""
...
def process_file(md_path: Path, failures: list[str]) -> bool:
    ...
    for m in FENCE_RE.finditer(original):
        ...
        snippet = Snippet(file=md_path, line=0, lang_tag="go", tab=None, code=code)
        run_result = run_go(snippet, timeout=TIMEOUT)
