const { OnlineLowessWasm } = require('../pkg/fastlowess_wasm.js');

/**
 * fastLowess WASM Online Smoothing Example
 * 
 * This example demonstrates online LOWESS smoothing for real-time data using WebAssembly:
...
function main() {
    console.log("=== fastLowess WASM Online Smoothing Example ===");
    ...
    // Note: WASM constructor takes (options, onlineOpts)
    const onlineFull = new OnlineLowessWasm(
        { fraction: 0.3, iterations: 3 },
        { windowCapacity: 50, updateMode: "full" }
    );
    const resFull = new Float64Array(nPoints);
    for (let i = 0; i < nPoints; i++) {
        const val = onlineFull.update(x[i], y[i]);
        // WASM update returns Option<f64> which is undefined/null in JS if None
        resFull[i] = (val !== undefined && val !== null) ? val : NaN;
    }

    // Incremental Update Mode (faster for large windows)
    console.log("Processing with 'incremental' update mode...");
    const onlineInc = new OnlineLowessWasm(
        { fraction: 0.3, iterations: 3 },
        { windowCapacity: 50, updateMode: "incremental" }
    );
    const resInc = new Float64Array(nPoints);
    for (let i = 0; i < nPoints; i++) {
        const val = onlineInc.update(x[i], y[i]);
        resInc[i] = (val !== undefined && val !== null) ? val : NaN;
    }
    ...
    console.log("\n=== WASM Online Smoothing Example Complete ===");
}
