...
GPU support is **opt-in** and not included in prebuilt C++ releases — download a prebuilt GPU-enabled build via the one-time installer below, or build from source with the `gpu` Cargo feature.
...
Before requesting `backend = "gpu"`, check whether the currently loaded library was built with GPU support. Requesting the GPU backend when it isn't available raises a clear error pointing at `fastlowess::gpu::install()`, rather than a raw panic.
...
If GPU support isn't available, requesting `backend = "gpu"` raises a runtime error pointing at `fastlowess::gpu::install()` rather than a raw Rust panic.
