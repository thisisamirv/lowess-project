//! Go bindings for fastLowess (via cgo).
//!
//! Provides C access to the fastLowess Rust library via C FFI, consumed by
//! the `bindings/go` Go package through cgo. The generated C header
//! (fastlowess_go.h) is the same shape as the C++ binding's, just with a
...
fn error_result_from(err: shared_parse::BindingError) -> GoLowessResult {
...
fn with_panic_result<F>(f: F) -> GoLowessResult
where
    F: FnOnce() -> GoLowessResult,
{
...
impl Default for GoLowessResult {
    fn default() -> Self {
        GoLowessResult {
            x: ptr::null_mut(),
            y: ptr::null_mut(),
...
fn error_result(msg: &str) -> GoLowessResult {
    GoLowessResult {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_lowess_new(
    fraction: c_double,
    iterations: c_int,
    delta: c_double,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_lowess_fit(
    ptr: *mut GoLowess,
...
pub unsafe extern "C" fn go_streaming_new(
    ...
    with_panic_ptr(|| {
        ...
        Box::into_raw(Box::new(GoStreamingLowess { model: Some(model) }))
    })
}
...
/// `ptr` must be valid.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_streaming_finalize(ptr: *mut GoStreamingLowess) -> GoLowessResult {
    with_panic_result(|| {
        if ptr.is_null() {
            return error_result(shared_parse::MODEL_POINTER_IS_NULL);
        }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_streaming_free(ptr: *mut GoStreamingLowess) {
    with_panic_void(|| {
        if !ptr.is_null() {
            let _ = Box::from_raw(ptr);
        }
...
pub unsafe extern "C" fn go_online_new(
    ...
    with_panic_ptr(|| {
        ...
        Box::into_raw(Box::new(GoOnlineLowess { model: Some(model) }))
    })
}
...
/// `ptr` must be valid or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_online_free(ptr: *mut GoOnlineLowess) {
    with_panic_void(|| {
        if !ptr.is_null() {
            let _ = Box::from_raw(ptr);
        }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_lowess_free_result(result: *mut GoLowessResult) {
    with_panic_void(|| {
        if result.is_null() {
            return;
        }
