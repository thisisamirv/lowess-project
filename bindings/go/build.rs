...
fn normalize_generated_header(output_file: &PathBuf) {
    ...
    let normalized = header
        ...
        .replace(
            "`ptr` must be a valid GoLowess pointer. `x` and `y` must be valid arrays of length `n`.",
            "`ptr` must be a valid GoLowess pointer. `x_values` and `y_values` must be valid arrays of length `n`.",
        )
        .replace(
