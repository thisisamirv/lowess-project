...
overlap. Points inside the overlap zone are fitted twice — once by the left
chunk and once by the right chunk. The `merge_strategy` decides how those two
...
Blends left- and right-chunk estimates using distance-based weights. Points
closer to the centre of each chunk get higher weight.
...
**Use when**: The left chunk has a larger context window and is expected to be
more accurate at the overlap boundary.
...
**Use when**: The right chunk is expected to provide better context (e.g., more
data follows).
