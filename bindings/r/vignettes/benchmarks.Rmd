...
For large batch datasets, the GPU backend can outperform CPU-parallel execution.
The crossover point depends on `fraction × n`:
...
around n ≥ 100K–250K. See the benchmarks README in the source repository
...
The benchmarks compare `rfastlowess` against `stats::lowess` using the
