...
> across overlap boundaries via `merge_strategy`, like `y`/`derivative`),
> and **Online** mode when `update_mode = "full"` is set (errors if
> combined with the default `"incremental"` mode).
...
> **Supported In All Three Adapters:** Confidence and prediction intervals
> are available in **Batch**, **Streaming**, and **Online** mode
> (`update_mode = "full"` only).
