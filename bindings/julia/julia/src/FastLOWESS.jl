...
platform. The GPU backend is not included in the default JLL binary; this
fetches a prebuilt library from the matching GitHub Release
...
on subsequent calls without re-downloading or re-prompting. Set
...
- `aic::Union{Float64, Nothing}`: Akaike Information Criterion, or `nothing`
	if not computed
- `aicc::Union{Float64, Nothing}`: Corrected AIC, or `nothing` if not computed
- `effective_df::Union{Float64, Nothing}`: Effective degrees of freedom, or
	`nothing` if not computed
...
- `delta::Float64 = NaN`: Interpolation optimization threshold (NaN auto-sets it to 1% of the x-range)
...
neighbourhood used at each point.
