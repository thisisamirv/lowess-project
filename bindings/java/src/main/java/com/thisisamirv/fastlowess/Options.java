...
/**
 * Configuration for a batch {@link Lowess} fit. Construct via {@link #builder()}.
 *
 * <p>{@link StreamingOptions} and {@link OnlineOptions} hold their own instance of the shared
 * subset of these settings (composition), plus settings specific to their execution mode.
 */
public final class Options {
    ...
    public static final class Builder {
        ...
        /** The fraction of points used to compute each local regression (default {@code 0.67}). */
        ...
        /** Skips recomputation for points within this distance of the last fit point (default {@code 0.0}). */
        public Builder delta(double delta) {
            this.delta = delta;
            return this;
        }

        /** One of {@code "tricube"}, {@code "tricube_symmetric"}, {@code "epanechnikov"}, {@code "gaussian"} (default {@code "tricube"}). */
        public Builder weightFunction(String weightFunction) {
            this.weightFunction = weightFunction;
            return this;
        }

        /** One of {@code "bisquare"}, {@code "huber"}, {@code "talworth"}, {@code "none"} (default {@code "bisquare"}). */
        public Builder robustnessMethod(String robustnessMethod) {
            this.robustnessMethod = robustnessMethod;
            return this;
        }

        /** One of {@code "mad"}, {@code "std"} (default {@code "mad"}). */
        public Builder scalingMethod(String scalingMethod) {
            this.scalingMethod = scalingMethod;
            return this;
        }

        /** One of {@code "extend"}, {@code "shrink"}, {@code "reflect"} (default {@code "extend"}). */
        public Builder boundaryPolicy(String boundaryPolicy) {
            this.boundaryPolicy = boundaryPolicy;
            return this;
        }

        /** How to handle all-zero local weight windows (default {@code "use_local_mean"}). */
        public Builder zeroWeightFallback(String zeroWeightFallback) {
            this.zeroWeightFallback = zeroWeightFallback;
            return this;
        }

        /** Stops iterating early once the relative change in fitted values drops below this value. */
        ...
        /** Whether to use the multi-threaded execution path (default {@code true}). */
        ...
        /** Candidate fractions to cross-validate; enables {@link Result#cvScores()}. */
        public Builder cvFractions(double[] cvFractions) {
            this.cvFractions = cvFractions;
            return this;
        }

        /** One of {@code "kfold"}, {@code "loo"} (default {@code "kfold"}); only used when {@code cvFractions} is set. */
        public Builder cvMethod(String cvMethod) {
            this.cvMethod = cvMethod;
            return this;
        }

        /** Number of folds for {@code "kfold"} cross-validation (default {@code 5}). */
