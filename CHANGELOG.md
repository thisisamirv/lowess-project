...
- Added a `return_derivative` option to the Batch adapter's builder: each point's local WLS fit already computes a slope internally, but only the fitted `y` was kept; `.return_derivative()` exposes that per-point slope in `LowessResult::derivative`, enabling rate-of-change/turning-point analysis at effectively no extra computation cost. For delta-skipped (interpolated) points, the slope is the constant slope of the linear segment connecting the neighboring anchor points, matching how `y` itself is interpolated there. `false` by default. Batch only.
...
- Added a Rayon-parallel `custom_derivative_pass` implementing the Batch adapter's new `return_derivative` option, mirroring the existing anchor-based parallel smoothing pass (including delta-skip/interpolation handling) and wired in alongside the other parallel passes when `.parallel(true)` is set.
