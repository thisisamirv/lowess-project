...
@testset "fastlowess Julia Bindings" begin
	...
	@testset "OnlineLowess" begin
		@testset "basic online" begin
			...
			results = filter(!isnothing, [add_point(online, xi, yi) for (xi, yi) ∈ zip(x, y)])
		...
		@testset "update modes" begin
			...
			results_full = filter(!isnothing, [add_point(o1, xi, yi) for (xi, yi) ∈ zip(x, y)])
			...
			results_inc = filter(!isnothing, [add_point(o2, xi, yi) for (xi, yi) ∈ zip(x, y)])
