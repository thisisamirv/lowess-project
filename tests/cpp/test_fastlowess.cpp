...
void assert_approx(double a, double b, double epsilon) {
    if (!is_approx(a, b, epsilon)) {
        std::cerr << "Assertion failed: " << a << " != " << b << " (eps=" << epsilon << ")" << std::endl;
        std::exit(1);
    }
}
...
void test_lowess_reuse() {
    ...
    auto r1 = lowess.fit(x1, y1);
    auto r2 = lowess.fit(x2, y2);
...
void test_streaming_returns_all_points() {
    ...
    auto r1 = stream.process_chunk(x, y);
    auto r2 = stream.finalize();

    assert_true(r1.y_vector().size() + r2.y_vector().size() == n, "Total points mismatch");
}
...
void test_streaming_basic() {
     ...
     auto r1 = stream.process_chunk(x, y);
     auto r2 = stream.finalize();
     // Just check no crash and valid objects
     // Note: r1 might be empty if chunk_size logic buffers? Wait, chunk_size=1000, input=2000. 
     // It processes chunks.
}
...
void test_online_basic() {
    ...
    for(size_t i=0; i<x.size(); ++i) {
        ...
        auto res = online.add_points(xi, yi);
