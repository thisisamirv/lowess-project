...
// False means do not compute or return the per-point local fit derivative (Batch only).
...
// False means `fit()` does not retain the fitted model state needed by `predict()` (Batch only).
