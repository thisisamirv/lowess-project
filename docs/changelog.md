...
- Fixed CRAN Windows build (`cannot find -lgcc_eh`): the Rtools45 posix-static toolchain does not ship `libgcc_eh.a`, causing `proc-macro2` and other build-script crates to fail to link. Fixed in `Makevars.win` by locating the GCC lib directory via `gcc -print-file-name=libgcc.a` and creating a dummy empty archive there before the `cargo build` step, so the link succeeds regardless of which drive Rtools is installed on.
- Fixed CRAN Windows build (`libgcc_eh.a: Permission denied`): on CRAN's Windows server the Rtools gcc lib directory is not writable, so the existing `ar rcs` fallback failed with a permission error. `Makevars.win` now falls back to creating the stub in `$(TARGET_DIR)` instead and injects `rustflags = ["-C", "symbol-mangling-version=v0", "-C", "link-arg=-L<path>"]` into the `[target.x86_64-pc-windows-gnu]` section of `.cargo/config.toml` via `awk`, so the linker finds the local stub.
