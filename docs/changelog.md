...
- Removed the test-copying step from `dev/prepare_cran.sh`. Tests now live directly in `bindings/r/tests/testthat/` and no longer need to be copied from a separate root-level `tests/r/` directory before `R CMD build`.
