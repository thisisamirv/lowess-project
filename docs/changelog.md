...
- Fixed `ld.lld` crashing (`Exception Code: 0xC0000005`) or dropping symbols entirely (`undefined symbol: WakeByAddressSingle`/`WaitOnAddress`) while linking `rfastlowess.dll` on Windows arm64/R-Universe. `--whole-archive` forces every crate's private raw-dylib stub object for a given DLL into the link, even when different crates' stubs cover different, non-overlapping symbols of that DLL (e.g. std's stub provides `WaitOnAddress`/`WakeByAddressAll`, `once_cell`'s provides `WakeByAddressSingle`); `--allow-multiple-definition` tolerates the resulting duplicates on x86_64 but crashes `ld.lld`'s arm64pe backend, and naively deduping same-named stub archive members to a single copy silently discarded symbols that only one of the duplicates provided. Fixed by dropping `--whole-archive`/`--no-whole-archive` entirely on the `gnullvm` target, letting normal single-definition-per-symbol archive resolution apply instead (nothing needed from the archive is lost, since `entrypoint.c` already references the extendr init symbol directly).
- Fixed CRAN Windows build failure (`error: linker ... not found`): `cargo-config.toml` hardcoded the MinGW linker and `ar` as absolute `c:/rtools45/...` paths, which don't exist on machines where Rtools is installed elsewhere (e.g. CRAN's Windows build server uses `d:/rtools45`). Fixed by referencing the tools by name only, resolved via `PATH`.
