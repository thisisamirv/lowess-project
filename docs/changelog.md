...
- Fixed `ld.lld` crashing (`Exception Code: 0xC0000005`) while linking `rfastlowess.dll` on Windows arm64/R-Universe, right after warning about duplicate `ProcessPrng`/`WakeByAddressAll` raw-dylib symbols. Multiple crates/CGUs each embed their own private stub object for the same Windows API symbol, and `--whole-archive` forces every copy into the link; `--allow-multiple-definition` tolerates the resulting duplicate definitions on x86_64 but crashes `ld.lld`'s arm64pe backend. Fixed by looping `ar d`/`ar r` over each `.dll` stub in `librfastlowess.a` to fully dedupe every occurrence of a given stub name before linking, instead of relying on the linker to tolerate the duplicates.
