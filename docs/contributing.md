...
- **Python build scripts**: Python is needed to build the R binding. The Makefile invokes Python scripts (`dev/prepare_cargo.py`, `dev/patch_vendor_crates.py`, `dev/clean_checksums.py`) during the vendoring step to read and rewrite TOML files. These scripts require `python3` with the `tomli` and `tomli_w` packages. The Makefile installs them automatically, falling back to `--user` mode for systems with an externally-managed Python (Ubuntu 24.04+, Debian 12+).
